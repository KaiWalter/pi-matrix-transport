...
This repository implements the default-OFF XO-only Matrix canary transport. Do not activate it, provision live credentials, alter XO settings, or modify a phone without explicit operator approval.
...
- Keep the sidecar Unix-socket-only and the Pi extension XO-only.
